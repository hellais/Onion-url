#!/usr/bin/python
# WARNING THIS CODE HAS NOT BEEN FULLY IMPLEMENTED
...
class mn:
    ...
    def words_required(self,size):
        return (size + 1) * 3 / 4
    
    def decode_next_word_index(ptr):
        
    def encode_word_index(self,data, n):
        
        if(n < 0 or n >= self.words_required(len(data))):
            return 0
        ...
        for i in range(0,remaining):
            x = x | data[offset + i] << (i * 8)
    
        if n % 3 == 2:
            if(remaining == 3):
                extra = self.BASE
            x = x / (self.BASE * self.BASE)
        elif n % 3 == 1:
            x = x/self.BASE
        return x % self.BASE + extra + 1
    
    def encode_word(self,data, n):
        return self.words[self.encode_word_index(data, n)]

    def decode_word_index(self, index, data, n): 
        groupofs = n & ~3
        x = 0
        for i in range(0,4):
            if(groupofs + i < len(data):
                x = x | data[groupofs + i] << (i * 8)
        if(index == self.EOF):
            if(n % 4 == 3):
                return self.OK
            elif(n % 4 == 2):
        ...
        if(n % 4 == 3):
            ...
            if(index >= self.BASE):
                ...
                if(n >= 1625 or n == 1624 and x > 1312671):
                    n = self.EENCODING
                    return n
                x = x + n * self.BASE * self.BASE
                n = n + 2
        elif(n % 4 == 1):
            x = x + n * self.BASE
            n = n + 1
    ...
    for i in range(0,4):
        if(groupofs + i < len(data)):
            data[groupofs + i] = x % 256
            x = x / 256
    return self.OK

a = mn()

test = "y6ynxr5s5v6ilui2"
text = base64.b32decode(test.upper())
final = []
tmpt = bytearray(text)
# Do a byte flip
...
for i in range(0, len(text)):
    final.append(str(a.encode_word(tmpt,i))+"-")
